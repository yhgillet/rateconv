...
public class Main {
  ...
  private static void output(ConvertResult result) {
    if (result.getValidationErrors().isEmpty()) {
      result.getResult().stream().map(Rate::toString).forEach(System.out::println);
    } else {
      result.getValidationErrors().forEach(e -> System.err.println(e.getMessage()));
    }
  }
}
